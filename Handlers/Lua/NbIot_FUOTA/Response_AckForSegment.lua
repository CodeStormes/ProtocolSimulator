...
local WITH_BLOCK_FEEDBACK = true
local expected_block = 0    -- 期望的上一块号
local received_block = 0    -- 实际收到的块号

local function build_result_tlv_with_block(result_code, expected, received)
    local tlv = { TAG_RESULT, 0x06, CHANNEL_ID, result_code, 0x03 }
    push_u16_le(tlv, expected)
    push_u16_le(tlv, received)
    return tlv
end
...
function build_command_response(request)
    ...
    if command_id ~= nil and command_id ~= 0x08 then
        error(string.format("Response_AckForSegment 收到 CommandID=0x%02X，请切换到对应脚本。", command_id))
    end

    local tlv
    if WITH_BLOCK_FEEDBACK then
        tlv = build_result_tlv_with_block(0x10, expected_block, received_block)
    else
        tlv = build_result_tlv(RESULT_SUCCESS)
    end

    local nb_packet = build_nb_uplink(tlv, mid)
    return wrap_long_frame(nb_packet)
end