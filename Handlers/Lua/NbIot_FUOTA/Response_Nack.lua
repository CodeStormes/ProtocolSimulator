...
-- 1 = 纯失败码；2 = 带 DataID 0x04（触发服务端按 AllowedImageDataLength 重新切片）
local RESULT_MODE = 1

local FAIL_CODE = 0x03      -- 0x03 Fail / 0x05 PacketLength / 0x0B ParameterLengthMismatch

local ALLOWED_IMAGE_DATA_LENGTH = 256   -- DataID 0x04 的 AllowedImageDataLen
local MAX_DOWNLOAD_TIME         = 1800  -- 单位：秒

local function build_nack_tlv()
    if RESULT_MODE == 2 then
        local tlv = { TAG_RESULT, 0x06, CHANNEL_ID, 0x10, 0x04 }
        push_u16_le(tlv, ALLOWED_IMAGE_DATA_LENGTH)
        push_u16_le(tlv, MAX_DOWNLOAD_TIME)
        return tlv
    end

    return build_result_tlv(FAIL_CODE)
end

function build_command_response(request)
    local _, mid = probe_request(request)
    local nb_packet = build_nb_uplink(build_nack_tlv(), mid)
    return wrap_long_frame(nb_packet)
end